...
func (s *SpecSchedule) Next(t time.Time) time.Time {
	...
	// increment the field until it matches. While incrementing the field, a
	// wrap-around brings it back to the beginning of the field list (since it
	// is necessary to re-verify previous field values).

	// Convert the given time into the schedule's timezone, if one is specified.
	// Save the original timezone so we can convert back after we find a time.
	// Note that schedules without a time zone specified ([time.Local]) are
	// treated as local to the time provided.
...
loop:
	...
	// Find the first applicable month. If it's this month, then do nothing.
	for 1<<uint(t.Month())&s.Month == 0 { //nolint:gosec
		// If we have to add a month, reset the other parts to 0.
		if !added {
			added = true
			// Otherwise, set the date at the beginning (since the current time
			// is irrelevant).
	...
	for !dayMatches(s, t) {
		...
		t = t.AddDate(0, 0, 1)
		// Notice if the hour is no longer midnight due to DST. Add an hour if
		// it's 23, subtract an hour if it's 1.
		if t.Hour() != 0 {
			if t.Hour() > 12 {
