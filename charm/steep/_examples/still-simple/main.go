...
// Glyphs from Nerd Fonts (PUA). TestGenerate uses SymbolsNerdFontMono by default;
// TestGenerateFira uses FiraMono Nerd Font Mono (see main_test.go).
const (
	nfTerminal = "\uf489" // oct_terminal
	nfGlobe    = "\uf0ac" // fa_globe
	nfBuilding = "\uf1ad" // fa_building
	nfChart    = "\uf080" // fa_bar_chart
	nfBranch   = "\ue0a0" // ply_git_branch
	nfSep      = "\ue0b0" // ply_right_divider
)
...
func (m model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.WindowSizeMsg:
		m.table.SetWidth(msg.Width - baseStyle.GetHorizontalFrameSize())
		m.table.SetHeight(msg.Height - baseStyle.GetVerticalFrameSize() - 2) // -2 for help view, and extra styling
		return m, nil
...
func (m model) View() tea.View {
	styleStrip := lipgloss.JoinHorizontal(lipgloss.Top,
		lipgloss.NewStyle().Bold(true).Render("bold"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Italic(true).Underline(true).UnderlineColor(lipgloss.Color("117")).Render("underline"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Underline(true).UnderlineStyle(lipgloss.UnderlineDouble).Foreground(lipgloss.Color("214")).Render("double"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Underline(true).UnderlineStyle(lipgloss.UnderlineCurly).Render("curly"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Strikethrough(true).Render("struck"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Faint(true).Render("faint"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("177")).Render(nfBranch+" nerdfont"),
		lipgloss.NewStyle().Faint(true).Render(" · "),
		lipgloss.NewStyle().Background(lipgloss.Color("236")).Foreground(lipgloss.Color("229")).Render(" "+nfSep+" "),
	)

	var view tea.View
	view.Content = baseStyle.Render(m.table.View()) + "\n " + m.table.HelpView() + "\n " + styleStrip + "\n"
	view.WindowTitle = nfTerminal + " Mega Cities"
	view.AltScreen = true
	view.BackgroundColor = lipgloss.Color("#0e1116")
	view.ForegroundColor = lipgloss.Color("#ffffff")
	return view
}

func newRankingModel() model {
	columns := []table.Column{
		{Title: lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("226")).Render(nfChart + " #"), Width: 5},
		{Title: lipgloss.NewStyle().Bold(true).Underline(true).UnderlineStyle(lipgloss.UnderlineDouble).Foreground(lipgloss.Color("117")).Render(nfBuilding + " City"), Width: 13},
		{Title: lipgloss.NewStyle().Italic(true).Foreground(lipgloss.Color("156")).Render(nfGlobe + " Country"), Width: 12},
		{Title: lipgloss.NewStyle().Strikethrough(true).Faint(true).Render(nfTerminal + " Pop."), Width: 12},
	}

	rows := []table.Row{
		{
			"1",
			lipgloss.NewStyle().Bold(true).Underline(true).UnderlineColor(lipgloss.Color("214")).Foreground(lipgloss.Color("229")).Render("Tokyo"),
			lipgloss.NewStyle().Italic(true).Render("Japan"),
			lipgloss.NewStyle().Faint(true).Render("37,274,000"),
		},
		{
			"2",
			lipgloss.NewStyle().Foreground(lipgloss.Color("212")).Render("Delhi"),
			lipgloss.NewStyle().Bold(true).Render("India"),
			lipgloss.NewStyle().Strikethrough(true).Faint(true).Render("32,065,760"),
		},
		{
			"3",
			lipgloss.NewStyle().Underline(true).UnderlineStyle(lipgloss.UnderlineDotted).Render("Shanghai"),
			"China",
			lipgloss.NewStyle().Italic(true).Faint(true).Render("28,516,904"),
		},
		{
			"4",
			lipgloss.NewStyle().Bold(true).Italic(true).Render("Dhaka"),
			lipgloss.NewStyle().Underline(true).Render("Bangladesh"),
			"22,478,116",
		},
		{
			"5",
			lipgloss.NewStyle().Faint(true).Render("São Paulo"),
			"Brazil",
			lipgloss.NewStyle().Bold(true).Foreground(lipgloss.Color("229")).Render("22,429,800"),
		},
		{
			"6",
			lipgloss.NewStyle().Underline(true).UnderlineStyle(lipgloss.UnderlineDashed).Render("Mexico City"),
			"Mexico",
			"22,085,140",
		},
